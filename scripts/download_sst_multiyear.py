...
def expected_timestamps(year: int) -> List[str]:
    ...
    return [(start + dt.timedelta(hours=i)).strftime("%Y-%m-%d %H:%M")
...
def values_from_payload(target: Dict[str, Any], payload: Dict[str, Any], expected: List[str]) -> List[Any]:
    ...
    if len(times) != len(expected) or len(values) != len(expected):
        raise RuntimeError(
            f"{target['city_name']} returned {len(values)} values / {len(times)} "
            f"timestamps, expected {len(expected)}."
        )
    if times != expected:
...
def main() -> int:
    ...
    ap.add_argument("--save-template", action="store_true",
                    help="Write a blank full-calendar-year SST template. Default: disabled.")
