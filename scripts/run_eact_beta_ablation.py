"""Run the beta-floor-zero EACT mechanism ablation under persistent shift."""
...
DEFAULT_OUT = os.path.join(ROOT, "results", "eact_beta_ablation_v1")
...
def build_jobs(args) -> list[Job]:
    ...
    for country in args.countries:
        tag = f"e4_beta0_shift_{country}_primary_shield_off"
        out_dir = os.path.join(args.out_root, country)
        command = (
            sys.executable,
            EVALUATOR,
            "--countries", country,
            ...
            "--adaptive-beta-floor", "0.0",
            "--weight-grid", "1.0",
            "--weight-co2", "2.0",
            "--weight-total", "0.2",
            "--weight-smooth", "0.5",
            "--forecast-stress", "adverse_bias",
            "--forecast-stress-scale", "1.0",
        ...
        jobs.append(Job("e4_beta_ablation", tag, command, out_dir, tag))
    return jobs
...
def main() -> None:
    ...
    print(
        f"beta-ablation jobs: total={len(jobs)} pending={len(pending)} "
    ...
    if failures:
        raise RuntimeError(f"Beta-ablation jobs failed: {failures}")
