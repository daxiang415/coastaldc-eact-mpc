"""Run the fixed ASHRAE inlet-temperature redesign pilot."""
...
DEFAULT_OUT = os.path.join(ROOT, "results", "ashrae_inlet_pilot_v1")
...
def build_jobs(args) -> list[Job]:
    ...
    if len(args.start_timestamps) != 4:
        raise ValueError("The pilot requires exactly four fixed seasonal starts")
    jobs = []
    for scenario in args.scenarios:
        scale = 0.0 if scenario == "none" else 1.0
        stage = "no_shift" if scenario == "none" else "adverse_bias_1sigma"
        for country in args.countries:
            tag = f"ashrae_inlet_{stage}_{country}_v1"
...
def main() -> None:
    ...
    print(
        f"ASHRAE inlet pilot jobs: total={len(jobs)} "
    ...
    if failures:
        raise RuntimeError(f"ASHRAE inlet pilot jobs failed: {failures}")
